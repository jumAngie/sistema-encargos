...
UPDATE tbCiudades
SET [ciu_Descripcion] = (SELECT UPPER(LEFT(ciu_Descripcion, 1)) + LOWER(SUBSTRING(ciu_Descripcion,2, 100)))
...
EXEC UDP_PEDIDOS_INSERT 'CSA012',1,350.00, 2, 11,'02-12-2023',1
...
AS
SELECT [ciu_ID] ID, deptos.depto_Descripcion [DEPARTAMENTO], [ciu_Descripcion] [NOMBRE CIUDAD] FROM [dbo].[tbCiudades] ciu
INNER JOIN [dbo].[tbDepartamentos] deptos
ON ciu.ciu_DeptoID = deptos.depto_ID
GO
...
AND
[usu_Clave] = @Pass AND usu_Estado = 1
END
...
----------------------------
go
CREATE OR ALTER PROCEDURE UDP_CargarDepartamentos
AS
BEGIN
	SELECT '0' AS depto_ID, ' Seleccione un Departamento' AS depto_Descripcion
	UNION ALL
...
--- TRIGGERS PARA ELIMINAR, CLIENTES - PEDIDOS - PEDIDOS DETALLES
--							EMPLEADOS - USUARIOS
go
...
BEGIN
	...
	BEGIN
		...
		WHERE	pedi_DireccionID IN (SELECT direc_ID FROM tbDirecciones WHERE direc_ClienteID = 
...
GO
CREATE OR ALTER TRIGGER tg_EliminarDirecciones 
ON tbClientes
FOR UPDATE
AS
BEGIN
	...
	BEGIN
			UPDATE tbDirecciones
			SET direc_Estado = 0
			WHERE direc_ClienteID IN (SELECT client_ID from inserted)
	END
END
...
CREATE OR ALTER   PROCEDURE [dbo].[UDP_CARGAR_ESTADOSCIV]
AS
BEGIN
SELECT [est_ID],[est_Descripcion] FROM [dbo].[tbEstadosCiviles]
END
...
-----agregar en la base datos archivos

ALTER PROCEDURE [dbo].[UDP_CARGAR_ESTADOSCIV]
AS
BEGIN
SELECT '0' AS est_ID, ' Seleccione un Estado Civil' AS est_Descripcion
	UNION ALL
...
--select*from [dbo].[tbEstadosCiviles]

--DElete from [dbo].[tbEstadosCiviles] where[est_Descripcion] = 'lll'

--DElete from [dbo].[tbClientes] WHERE [client_ID] = 36
